  ...
  update: protectedProcedure
    ...
    .mutation(async({ ctx, input  }) => {
      ...
      return await prisma.$transaction(async(tx) => {
        await tx.node.deleteMany({  // Borramos los nodos existentes
          where: { workflowId: id }
        });

        await tx.node.createMany({  // Creamos los nodos nuevos
          data: nodes.map((node) => ({
            id: node.id,
            workflowId: id,
        ...
        await tx.connection.createMany({ // Creamos las conexiones nuevas
          data: edges.map((edge) => ({
            workflowId: id,
        ...
        await tx.workflow.update({  // Actualizamos en el workflow el timestamp de la última actualización
          where: { id },
          data: { updatedAt: new Date() }
        });

        return workflow;  // Devolvemos el workflow actualizado
      })
    }),
